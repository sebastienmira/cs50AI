...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        if available_actions:
            for action in Nim.available_actions(state):
                qvalues.append(self.get_q_value(state, action))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in available_actions:
            qvalues.update({action: self.get_q_value(state, action)})

        qvalues = sorted(qvalues.items(), key = lambda x: -x[1])
             
        if epsilon == False:
            return list(list(qvalues)[0])[0]   
